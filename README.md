...
The ideas of this game is To collect the cubes scattered across the desk and put them in the right box is the game's objective. The cubes' positions serve as markers for the targets, making the range of motion variable and easily adaptable to each patient. The cursor has control over the manipulator's end effector and can move it in a plane parallel to the desk. The end effector lowers in the last position it was in on the left click event and can either pick up a cube or deposit it into a box. A chosen cube can only be dropped inside the box region after being picked. To get points and finish the game, drop all the cubes into the proper box.
